...
router.post('/:id/judicial-reviews/sync', authMiddleware('ADMIN'), async (req: Request, res: Response) => {
  try {
    ...
    return res.status(202).json({
      success: true,
      taskId: task.id,
      message: 'Sinkronisasi putusan judicial review sedang berjalan di latar belakang.'
    });
  } catch (error) {
    logger.error('Error queuing judicial review sync:', error);
    return res.status(500).json({ success: false, error: 'Failed to queue judicial review sync' });
  }
});
